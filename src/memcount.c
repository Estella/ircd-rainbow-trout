...
void report_memory_usage(aClient *cptr, int detail)
{
     ...
     */
    if (detail)
      sendto_one(&me, cptr, "%sClients", pfxbuf);
    subtotal = 0;
    if (detail && mc_s_user.e_local_clients)
      sendto_one(&me, cptr, "%s    local clients: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_remote_clients)
      sendto_one(&me, cptr, "%s    remote clients: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_flud_links)
      sendto_one(&me, cptr, "%s    flud links: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_channel_links)
      sendto_one(&me, cptr, "%s    channel links: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_invite_links)
      sendto_one(&me, cptr, "%s    invite links: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_silence_links)
      sendto_one(&me, cptr, "%s    silence links: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_dccallow_links)
      sendto_one(&me, cptr, "%s    dccallow links: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_watch_links)
      sendto_one(&me, cptr, "%s    client-watch links: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_hash.e_links)
      sendto_one(&me, cptr, "%s    watch-client links: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_clientlist.e_oper_dlinks)
      sendto_one(&me, cptr, "%s    operlist dlinks: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_clientlist.e_recvq_dlinks)
      sendto_one(&me, cptr, "%s    recvqlist dlinks: %d (%lu bytes)", pfxbuf,
    ...
    if (detail)
      sendto_one(&me, cptr, "%s    TOTAL: %lu bytes", pfxbuf, subtotal);
    else
      sendto_one(&me, cptr, "%sClients: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
     ...
     */
    if (detail)
      sendto_one(&me, cptr, "%sServers", pfxbuf);
    subtotal = 0;
    if (detail && mc_s_user.servers.c)
      sendto_one(&me, cptr, "%s    servers: %d (%lu bytes)", pfxbuf,
                   mc_s_user.servers.c, mc_s_user.servers.m);
    subtotal += mc_s_user.servers.m;
    if (detail && mc_clientlist.e_server_dlinks)
      sendto_one(&me, cptr, "%s    serverlist dlinks: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_dh_sessions)
      sendto_one(&me, cptr, "%s    DH sessions: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_rc4states)
      sendto_one(&me, cptr, "%s    RC4 states: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_zipin_sessions)
      sendto_one(&me, cptr, "%s    zip input sessions: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_s_user.e_zipout_sessions)
      sendto_one(&me, cptr, "%s    zip output sessions: %d (%lu bytes)", pfxbuf,
    ...
    if (detail)
      ...
      sendto_one(&me, cptr, "%sServers: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
     ...
     */
    if (detail)
      sendto_one(&me, cptr, "%sChannels", pfxbuf);
    subtotal = 0;
    if (detail && mc_channel.e_channels)
      sendto_one(&me, cptr, "%s    channels: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_channel.e_lopt_links)
      sendto_one(&me, cptr, "%s    lopt links: %d (%lu bytes)", pfxbuf,
    ...
    if (detail && mc_channel.e_dlinks)
      sendto_one(&me, cptr, "%s    dlinks: %d (%lu bytes)", pfxbuf,
                   mc_channel.e_dlinks,
    ...
    if (detail)
      ...
      sendto_one(&me, cptr, "%sChannels: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
    ...
    if (detail)
      ...
      sendto_one(&me, cptr, "%sBans: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
    ...
    if (detail)
      ...
      sendto_one(&me, cptr, "%sConfiguration: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
     ...
     */
    if (detail)
      sendto_one(&me, cptr, "%sClones", pfxbuf);
    subtotal = 0;
    if (detail && mc_clones.e_cloneents)
      sendto_one(&me, cptr, "%s    clone entries: %d (%lu bytes)", pfxbuf,
    ...
    if (detail)
      ...
      sendto_one(&me, cptr, "%sClones: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
     ...
     */
    if (detail)
      sendto_one(&me, cptr, "%sThrottles", pfxbuf);
    subtotal = 0;
    if (detail && mc_throttle.e_throttles)
      sendto_one(&me, cptr, "%s    throttle entries: %d (%lu bytes)", pfxbuf,
    ...
    if (detail)
      ...
      sendto_one(&me, cptr, "%sThrottles: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
     ...
     */
    if (detail)
      sendto_one(&me, cptr, "%sMiscellaneous", pfxbuf);
    subtotal = 0;
    if (detail && mc_hide.fakelinks.c)
      sendto_one(&me, cptr, "%s    fakelink servers: %d (%lu bytes)", pfxbuf,
                   mc_hide.fakelinks.c, mc_hide.fakelinks.m);
    subtotal += mc_hide.fakelinks.m;
    if (detail && mc_hide.e_links)
      sendto_one(&me, cptr, "%s    fakelink links: %d (%lu bytes)", pfxbuf,
    ...
    if (detail)
      ...
      sendto_one(&me, cptr, "%sMiscellaneous: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
     ...
     */
    if (detail)
      sendto_one(&me, cptr, "%sBlock Allocators", pfxbuf);
    subtotal = 0;
    if (detail)
    {
      sendto_one(&me, cptr, "%s    local clients:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
      ...
      sendto_one(&me, cptr, "%s        overhead: %lu bytes"
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    remote clients:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    users:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    channels:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    channel members:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    links:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    dlinks:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    fludbots:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    clones:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    throttles:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    hash entries:", pfxbuf);
      sendto_one(&me, cptr, "%s        objects: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
      ...
      sendto_one(&me, cptr, "%sBlock heaps: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
     ...
     */
    if (detail)
      sendto_one(&me, cptr, "%sShared Buffers", pfxbuf);
    subtotal = 0;
    if (detail)
      sendto_one(&me, cptr, "%s    small buffers: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
                   mc_sbuf.smallbufs.c, mc_sbuf.smallbufs.m,
                   mc_sbuf.smallbufpool.c, mc_sbuf.smallbufpool.m);
    subtotal += mc_sbuf.smallbufpool.m;
    if (detail)
      sendto_one(&me, cptr, "%s    large buffers: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
    ...
    if (detail)
    {
      sendto_one(&me, cptr, "%s    overhead: %lu bytes", pfxbuf,
                   mc_sbuf.management.m);
      sendto_one(&me, cptr, "%s        headers: %d (%lu bytes)", pfxbuf,
                   mc_sbuf.bufheaders.c, mc_sbuf.bufheaders.m);
      sendto_one(&me, cptr, "%s        users: %d (%lu bytes)"
                   " [pool: %d (%lu bytes)]", pfxbuf,
                   mc_sbuf.users.c, mc_sbuf.users.m,
                   mc_sbuf.userpool.c, mc_sbuf.userpool.m);
      sendto_one(&me, cptr, "%s        user blocks: %d (%lu bytes)", pfxbuf,
                   mc_sbuf.userblocks.c, mc_sbuf.userblocks.m);
      sendto_one(&me, cptr, "%s        buffer blocks: %d (%lu bytes)", pfxbuf,
    ...
    if (detail)
      ...
      sendto_one(&me, cptr, "%sShared buffers: %lu bytes", pfxbuf, subtotal);
    rep_total += subtotal;
    ...
    if (detail)
    {
      ...
      sendto_one(&me, cptr, "%s    read buffer: %lu bytes", pfxbuf,
        ...
        subtotal += mc_s_bsd.s_readbuf.m;
#endif
      sendto_one(&me, cptr, "%s    send buffers: %d (%lu bytes)", pfxbuf,
                   mc_send.s_bufs.c, mc_send.s_bufs.m);
        subtotal += mc_send.s_bufs.m;
      sendto_one(&me, cptr, "%s    zip buffers: %d (%lu bytes)", pfxbuf,
  ...
  sendto_one(&me, cptr, "%s-----", pfxbuf);

    if (rep_total != alloc_total)
    {
      sendto_one(&me, cptr, "%sUNREPORTED: %ld bytes", pfxbuf,
    ...
    if (mcbh_clones.knownobjs != mcbh_clones.objects.c)
    {
      ...
      sendto_one(&me, cptr, "%sLEAK: %d objects (%lu bytes)"
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from block allocator", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from channels", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from watch hash", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from fake links", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from confs/servers", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from parser", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from resolver", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from listeners", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from users", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from sbuf", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from scache", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from generic hash", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from zlink", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from DH sessions", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from RC4 states", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sLEAK: %ld bytes from conf/motd/bans", pfxbuf,
    ...
    {
      sendto_one(&me, cptr, "%sWARNING: %lu bytes in %d objects have not been"
    ...
    /* the grand summary */
  sendto_one(&me, cptr, "%sTOTAL: %lu bytes (%lu active, %lu cached)",
  ...
  sendto_one(&me, cptr, "%sTraced %lu bytes (%ld leaked)", pfxbuf,
               traced_total, traced_total - alloc_total);
  sendto_one(&me, cptr, "%sTracer overhead is %lu bytes", pfxbuf, subtotal);

    memtrace_reset();
#endif  /* MEMTRACE */

  sendto_one(&me, cptr, "%ssbrk(0)-etext: %lu", pfxbuf,
