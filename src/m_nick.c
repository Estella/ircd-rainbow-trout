...
int m_enick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	if (parc >= 10)
	{
	    ...
	    if (parc==13)
	    {
                (void) strncpy(sptr->user->realhost, parv[10], HOSTLEN); // ENICK strcpying realhost to realhost's place ;p - janicez
                (void) strncpy(sptr->user->mangledhost, parv[11], HOSTLEN); // ENICK strcpying realhost to realhost's place ;p - janicez
		return do_user(nick, cptr, sptr, parv[5], parv[6],
			       parv[7], strtoul(parv[8], NULL, 0),
			       parv[9], parv[12], NULL);
	    }
