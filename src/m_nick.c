...
int m_enick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
    struct simBan *ban;
    aClient    *acptr, *uplink;
    Link       *lp, *lp2;
	...
	parc = 2;
    if (!IsServer(cptr)) return 0; // ENICK is only used on S2S connectons.
    /*
     * parc == 2 on a normal client sign on (local) and a normal client 
     * nick change 
     * parc == 4 on a normal server-to-server client nick change
     * parc == 13 on a normal TS style server-to-server ENICK introduction
     */
    if ((IsServer(sptr) || (parc > 4)) && (parc < 11))
    {
	/*
	 ...
	 * and continue -Dianora
	 */
	sendto_realops("IGNORING BAD ENICK: %s[%s@%s] on %s (from %s)", parv[1],
		       (parc >= 6) ? parv[5] : "-",
		       (parc >= 7) ? parv[6] : "-",
		       (parc >= 8) ? parv[7] : "-", parv[0]);
	return 0;
	...
	if (parc >= 10)
	{
	    ...
	    if (parc==13)
	    {
                (void) strncpy(sptr->user->realhost, parv[9], HOSTLEN); // ENICK strcpying realhost to realhost's place ;p - janicez
                (void) strncpy(sptr->user->mangledhost, parv[10], HOSTLEN); // ENICK strcpying realhost to realhost's place ;p - janicez
		return do_user(nick, cptr, sptr, parv[5], parv[6],
			       parv[7], strtoul(parv[8], NULL, 0),
			       "0.0.0.0", parv[11], NULL);
	    }
	}
    }
    else if (sptr->name[0])
