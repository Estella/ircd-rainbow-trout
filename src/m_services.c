...
hostchange_qjm (aClient *cptr, char *oldhost)
{
    ...
    sendto_common_channels_butfrom(cptr, ":%s!%s@%s QUIT :Signed on (SVSHOST: %s)", get_client_name(cptr, FALSE), cptr->user->username, oldhost, cptr->user->host);

    for (clink = cptr->user->channel; clink; clink = clink->next) {
        sendto_channel_butone_local(cptr, cptr, clink->value.chptr, ":%s!%s@%s JOIN %s", get_client_name(cptr, FALSE), cptr->user->username, cptr->user->host, clink->value.chptr->chname);
        if (is_chan_superop(cptr, clink->value.chptr)) {
            *mb = 'a';
            strcat(mvb, get_client_name(cptr, FALSE));
            strcat(mvb, " ");
        }
        ...
        mb = mode;
        sendto_channel_butone_local(cptr, cptr, clink->value.chptr, ":%s!%s@%s MODE %s +%s %s", get_client_name(cptr, FALSE), cptr->user->username, cptr->user->host, clink->value.chptr->chname, mb, mvb);
    }
}
